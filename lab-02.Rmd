...
author: "Team Name: Finn, Tom, Roman and User4 Scott "
date: "`r Sys.Date()`"
...
```
#Africa has the most terrotories
...
plastic_data_all %>%  
  group_by(region) %>%
  summarise(avg_plastic = mean(total_mismanaged_plastic))

plastic_data_all %>%  
  group_by(region) %>%
  summarise(max_plastic = max(total_mismanaged_plastic))

plastic_data_all %>%  
  group_by(region) %>%
  summarise(min_plastic = min(total_mismanaged_plastic))
```
...
group_by(region)%>%
  summarise(median_oceanwaste = median(pct_mismanaged_plastic_ocean))


```
#na present in Africa data
